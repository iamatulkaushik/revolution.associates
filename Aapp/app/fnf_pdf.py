...
Aapp/app/fnf_pdf.py
======================
PDF generators for Full & Final Settlement:
  fnf_settlement_pdf(settlement)          -> settlement statement, print/record
  fnf_certificate_pdf(settlement, type)   -> Experience / Last Pay / Character certificate
"""
...
def fnf_settlement_pdf(settlement):
    styles = doc_styles()
...
def fnf_certificate_pdf(settlement, cert_type):
    """cert_type: 'experience' | 'last_pay' | 'character'"""
    emp = settlement.employee
    styles = doc_styles()
    ...
    if cert_type == 'experience':
        ...
        body = (
            f"This is to certify that {emp.name} (Employee Code: {emp.employeecode}) was employed "
            f"with this organization from {emp.dateofjoining.strftime('%d-%m-%Y') if emp.dateofjoining else '-'} "
            f"to {settlement.last_working_day.strftime('%d-%m-%Y')}. "
            f"During this period, their conduct and performance were found satisfactory. "
            f"We wish them success in their future endeavours."
        )
    elif cert_type == 'last_pay':
        title = "Last Pay Certificate"
        body = (
            f"This is to certify that {emp.name} (Employee Code: {emp.employeecode}) has been paid "
        ...
        body = (
            f"This is to certify that {emp.name} (Employee Code: {emp.employeecode}) has, to the best "
            f"of our knowledge, maintained good conduct, punctuality, and behaviour during their tenure "
            f"with this organization from {emp.dateofjoining.strftime('%d-%m-%Y') if emp.dateofjoining else '-'} "
            f"to {settlement.last_working_day.strftime('%d-%m-%Y')}. No disciplinary action was pending "
